...
public class DocumentTaggingService implements DocumentTaggingHandler {
    ...
    @Override
    @Retryable(
            value = DocumentIdNotFoundException.class
    )
    ...
    private void processDocumentDepartmentTags(String fileKey, Map<String, String> documentTags){
        ...
        List<String> documentDepartmentList = departmentsString == null ? Collections.emptyList()
                : Arrays.stream(departmentsString.split("/"))
                .filter(s -> !s.isBlank())
                .map(String::trim)
        ...
        if(documentDepartmentList.isEmpty()){
            ...
            documentDepartmentSyncService.removeDocumentDepartments(fileKey, documentDepartmentList);
            return;
        }
